...
sub _parse_columns {
  ...
  foreach my $col (@cols) {
    ...
    # bare column; quote if required
        ...
        $col = $self->_quote($col) if $quote_name && _quotable_column( $col );
    }
  }
...
sub _parse_table {
  ...
  $args{quote_aliased_name} = 1 if ! defined $args{quote_aliased_name};
  $args{quote_name} = 0 if ! defined $args{quote_name};
  ...
  return {
    ...
    is_literal     => $args{quote_name} || defined $alias,
   };
}
...
sub _make_sub_column_alias {
  ...
  $self->{column_alias} = sub {
    ...
    # quote $name unless it is an SQL expression (then the user should quote it)
    $name = $self->_quote($name) if $quote_name && _quotable_column( $name );
...
sub _make_sub_table_alias {
  ...
  $self->{table_alias} = sub {
    my ($self, $name, $alias, %args ) = @_;

    $args{quote_aliased_name} = 1 if ! defined $args{quote_aliased_name};
    $args{quote_name} = 1 if ! defined $args{quote_name};
